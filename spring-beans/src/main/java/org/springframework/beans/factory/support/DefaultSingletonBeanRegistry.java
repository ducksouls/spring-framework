...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
    ...
    /**
     * 一级缓存 用beanName 指向 bean实例
     */
    private final Map<String, Object> singletonObjects = new ConcurrentHashMap<>(256);

    /** Cache of singleton factories: bean name to ObjectFactory. */
    /**
     * 三级缓存 用beanName 指向 bean的工厂
     */
    private final Map<String, ObjectFactory<?>> singletonFactories = new HashMap<>(16);

    /** Cache of early singleton objects: bean name to bean instance. */
    /**
     * 二级缓存	用beanName 指向 bean实例
     */
    ...
    protected void addSingleton(String beanName, Object singletonObject) {
        synchronized (this.singletonObjects) {// bean创建完毕, 也执行完了后置处理...
    ...
    protected Object getSingleton(String beanName, boolean allowEarlyReference) {
        ...
        if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
            ...
            if (singletonObject == null && allowEarlyReference) { // 这个才是最重要的判断...
                synchronized (this.singletonObjects) {
                    ...
                    if (singletonObject == null) {
                        ...
                        if (singletonObject == null) {
                            ...
                            if (singletonFactory != null) { //骚操作...把3级删掉,存入2级...为什么呢?
        ...
        synchronized (this.singletonObjects) {
            ...
            if (singletonObject == null) {
                if (this.singletonsCurrentlyInDestruction) {
                    throw new BeanCreationNotAllowedException(beanName, "Singleton bean creation not allowed while " +
                ...
                beforeSingletonCreation(beanName);// 把beanName加入InCreation中
                boolean newSingleton = false;
                boolean recordSuppressedExceptions = (this.suppressedExceptions == null);
                if (recordSuppressedExceptions) {
                    this.suppressedExceptions = new LinkedHashSet<>();
                }
                try {//这里才是真的创建bean的地方, 妈的...跟了这么久
                    singletonObject = singletonFactory.getObject();
                    newSingleton = true;
                }
                ...
                if (newSingleton) {
                    addSingleton(beanName, singletonObject);// 加入singleton中
                }
            }
            return singletonObject;
        }
